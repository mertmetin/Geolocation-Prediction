...
with open('coordinates_province.json') as f:
    coordinates_provinces = json.load(f)

with open('city_and_provinces.json') as f:
    city_and_provinces = json.load(f)
...
for category in province_categories:
    D[category] = 0
...
for user in users:
    screen_name = user[3]
    user_location = user[10]
    followers = twts.get_followers(screen_name)
    ...
    for follower in followers:
        follower_location = remove_emoji(follower[5])        

        if follower_location:    
            places = GeoText(follower_location)
            city = places.cities
            ...
            if city:
                ...
                if metro in categories:
                    province = city_and_provinces[metro]
                    cpy[province] = cpy[province] + 1
                elif metro in province_categories:
                    cpy[metro] = cpy[metro] + 1
                else:
                    if len(places.cities) == 2 and places.cities[1] in province_categories:
                        province = places.cities[1]
                        cpy[province] = cpy[province] + 1


    followers_locations.append(cpy)


vec = DictVectorizer(sparse=False)
X = vec.fit_transform(followers_locations)
...
for value, target in zip(X_test, y_test):
    ...
    # print(text_clf.predict([value]))

    metro_d = coordinates_provinces[target]
    predicted_d = coordinates_provinces[predicted]
